...
class Model(object):
    ...
    def train(self, source_train, target_train, source_ul, target_ul, dev, test):
        ...
        s_get_loss_and_pred = theano.function(
            ...
            outputs=[self.s_lab_prob, self.s_lab_loss, self.s_rel_loss, self.s_dom_loss, self.s_adv_loss,
                     self.s_recon_loss]
        )
        t_get_loss_and_pred = theano.function(
            inputs=[self.t_idxs, self.t_idys, self.t_gold_rels, self.t_dom_ids],
            outputs=[self.t_lab_prob, self.t_lab_loss, self.t_rel_loss, self.t_dom_loss, self.t_adv_loss,
                     self.t_recon_loss]
        )
        ...
        for epoch in xrange(args.epochs):
            ...
            if dev:
                ...
                dev_lab_loss, dev_rel_loss, dev_dom_loss, dev_adv_loss, dev_recon_loss, dev_acc, dev_f1 = self.evaluate_data(
                    dev_batches, s_get_loss_and_pred)
                ...
                say((
                "\tdev_lab_loss={:.4f}  dev_rel_loss={:.4f}  dom_loss={:.4f}  adv_loss={:.4f}  recon_loss={:.4f}  dev_acc={:.4f}  dev_f1={}" +
                "  best_dev={:.4f}\n").format(
                    dev_lab_loss,
                    dev_rel_loss,
                    dev_dom_loss,
            ...
            if test:
                self.dropout.set_value(0.0)
                self.cnn_layer.set_runmode(1)
                test_lab_loss, test_rel_loss, test_dom_loss, test_adv_loss, test_recon_loss, test_acc, test_f1 = self.evaluate_data(
                    test_batches, t_get_loss_and_pred)
                self.dropout.set_value(dropout_prob)
                self.cnn_layer.set_runmode(0)
                say((
                "\ttest_lab_loss={:.4f}  test_rel_loss={:.4f}  dom_loss={:.4f}  adv_loss={:.4f}  recon_loss={:.4f}  test_acc={:.4f}  test_f1={}\n").format(
                    test_lab_loss,
                    test_rel_loss,
                    test_dom_loss,
                    test_adv_loss,
